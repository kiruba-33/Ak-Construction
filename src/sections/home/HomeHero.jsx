...
export default function HomeHero() {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Initial State
      gsap.set(
        [bgTextRef.current, subtitleRef.current, ".info-bottom"],
        { opacity: 0, y: 50 }
      );
  ...
  return (
    <section
      ref={containerRef}
      className="relative h-screen w-full overflow-hidden font-sans bg-[#050505] text-white flex items-center justify-center"
    >
  ...
  playsInline
  preload="metadata"
  className="h-full w-full object-cover opacity-75 relative z-0"
>
  <source src="/hero-video.webm" type="video/webm" />
  <source src="/hero-video.mp4" type="video/mp4" />
</video>

        {/* SAME FADE AS ABOUT HERO */}
      ...
      <div className="absolute inset-0 flex items-center justify-center z-30 pointer-events-none">
        ...
        <motion.div
          style={{ y: textY, x: translateX, y: translateY }}
      ...
      <div className="absolute bottom-8 sm:bottom-12 left-0 w-full px-6 sm:px-12 flex flex-col sm:flex-row justify-between items-center sm:items-end z-20 gap-6 sm:gap-0">
        ...
        <div className="info-bottom hidden sm:block">
          <div className="flex items-center gap-4">
            ...
            <p className="text-[7px] md:text-[8px] font-bold tracking-[0.5em] text-white/30 uppercase">
        ...
        <div className="info-bottom max-w-[280px] sm:max-w-xs text-center sm:text-right">
          <p className="text-[8px] md:text-[9px] font-bold tracking-[0.2em] text-orange-500 uppercase mb-1 sm:mb-2">
            Architecture with meaning
          </p>

          <p className="text-[8px] md:text-[9px] leading-relaxed text-white/30 font-medium">
            Bespoke architectural solutions crafted with precision.
            We build beyond structures, creating living spaces that inspire.
          </p>
        </div>
